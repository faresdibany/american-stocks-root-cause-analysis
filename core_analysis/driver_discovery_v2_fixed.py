"""
driver_discovery_v2.py (FIXED)

Outcome-driven driver discovery (v2, fixed):
- Uses VARIANCE attribution (beta_std^2) instead of mean(|beta * r|) contributions.
- Prunes factor set further to reduce multicollinearity (removes VIX and extra style proxies from discovery).
- Keeps standardized ridge for stable beta estimation.
- Provides market/sector proxies + a full (but pruned) factor dict suitable for downstream RCA.

Why this fix?
-------------
In the previous v2, most large-cap stocks were incorrectly labeled as
"narrative_or_idiosyncratic" with ~90% idiosyncratic share because:
- VIX (high vol) could dominate "contribution" metrics, and
- too many correlated market-like factors diluted betas.

This version computes driver shares from explained variance on standardized data:
  explained_var_k ∝ beta_std_k^2  (since Var(Xz_k)=1)
  idio_var ∝ Var(residual_z)

So market/sector should correctly dominate for mega-caps/BRK-B, etc.

Interfaces (unchanged)
----------------------
- infer_metadata(ticker) -> dict
- build_candidate_factors(meta) -> dict[str,str]
- discover_driver_profile(ticker, start, end, *, ...) -> dict
"""
...
# Discovery factor set: keep *small* and interpretable
# (You can still pass richer factor dict downstream; discovery should be stable.)
DEFAULT_FACTORS_DISCOVERY: Dict[str, str] = {
    "rates_long": "TLT",
    "usd": "UUP",
    "oil": "USO",
    "gold": "GLD",
...
def build_candidate_factors(meta: Dict[str, Any]) -> Dict[str, str]:
    """
    Returns a factor dict:
    - Always: market
    - If available: sector
    - If non-US and market is a country ETF: add ONE of {region, global} (not both) to reduce collinearity.
    - Adds a small macro set (rates_long, usd, oil, gold).
    """
    ...
    region = _infer_region(meta)
    # Add a single decomposer (region OR global) for non-US
    if region in {"europe","asia_pacific","emerging"}:
        factors["region"] = REGION_DEFAULTS[region]
    elif market != REGION_DEFAULTS["global"]:
        factors["global"] = REGION_DEFAULTS["global"]

    factors.update(DEFAULT_FACTORS_DISCOVERY)
    return factors
...
def _ridge_fit_std(Xz: np.ndarray, yz: np.ndarray, alpha: float) -> np.ndarray:
    """Ridge on standardized data, no intercept (standardized)."""
    XtX = Xz.T @ Xz
...
def discover_driver_profile(
    ...
    *,
    ...
    Key change vs previous v2:
    - Driver shares are computed via variance attribution on standardized data:
        share_k ∝ beta_std_k^2
        idio_share ∝ Var(residual_z)
    """
    ...
    # ----------------------------
    # VARIANCE attribution (FIX)
    # ----------------------------
    explained_var = betas_std ** 2  # since Var(Xz_k)=1
    idio_var = float(np.var(resid_z))

    total_var = float(np.sum(explained_var) + idio_var)
    if total_var <= 1e-12:
        shares = np.zeros_like(explained_var)
        idio_share = 0.0
    else:
        shares = explained_var / total_var
    ...
    # Mode inference (robust + interpretable)
    mode = "mixed"
    if market_like >= 0.45:
        mode = "beta_or_sector_driven"
    elif share_map.get("rates_long", 0.0) >= 0.20:
        mode = "rates_sensitive"
    elif share_map.get("oil", 0.0) >= 0.20:
        mode = "commodity_oil_sensitive"
    elif idio_share >= 0.55:
    ...
    if (mcap is not None and mcap >= 5e10) and r2 < 0.15:
        out["warnings"].append("low_r2_for_large_cap: driver discovery may be unreliable; treat mode and shares cautiously.")
    ...
    out.update({
        ...
        "idiosyncratic": {"share": float(idio_share), "variance": float(idio_var)},
        ...
        "notes": [
            "FIX: driver shares computed via variance attribution on standardized data (beta_std^2).",
            "Discovery factor set is intentionally small to reduce collinearity and stabilize mode inference.",
